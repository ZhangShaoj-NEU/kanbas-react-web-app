...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
        ...
        <input defaultValue={todo.id} className="form-control w-50" onChange={(e) => setTodo({ ...todo, id: Number(e.target.value)})}/><hr/>
        ...
        <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: Number(e.target.value) })}/>
        <input defaultValue={todo.description} className="form-control w-50 float-start me-2"
        onChange={(e) => setTodo({ ...todo, description: e.target.value }) }/>
        <br/><br/><hr/>
        <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
            Update Todo Complete State
        </a>
        <input defaultValue={todo.id} className="form-control w-25 float-start"
        onChange={(e) => setTodo({ ...todo, id: Number(e.target.value) })}/>
        <div className="form-check float-start">
        <input className="form-check-input" id="wd-todo-completed" type="checkbox"
